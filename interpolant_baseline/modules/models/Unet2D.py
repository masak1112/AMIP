...
class Unet2d(nn.Module):
    ...
    def __init__(
        ...
        self.in_proj = nn.Conv2d(self.in_channels, 
        ...
        self.out_proj = zero_module(nn.Conv2d(in_channels, self.out_channels, kernel_size=1))
        
        if self.cond_dim > 0:
            self.cond_emb = FourierEmb(hidden_dim=self.cond_channels, in_dim=self.cond_dim)
            self.use_cond = True
        else:
            self.use_cond = False 
    ...
    def forward(self, 
        ...
        Args:
            ...
            emb (torch.Tensor): condition tensor of shape [batch, cond_dim]
        ...
        if self.use_cond:
            emb = self.cond_emb(emb) # (b, cond_dim) -> (b, cond_channels)
         
        x = self.in_proj(x) # (batch, c, x, y) -> (batch, hidden_channels, x, y)
        ...
        x = self.out_proj(self.activation(self.norm(x)))
