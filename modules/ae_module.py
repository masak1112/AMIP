...
class AutoencoderModule(L.LightningModule):
    ...
    def _make_muon_closure_compatible(self, optimizer):
        """Lightning calls optimizer.step(closure=...).

        Some Muon implementations don't accept a closure kwarg, so we execute
        the closure ourselves and delegate to the original step.
        """
        try:
            step_sig = inspect.signature(optimizer.step)
            accepts_closure = (
                "closure" in step_sig.parameters
                or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in step_sig.parameters.values())
            )
        ...
        def step_with_closure(*args, closure=None, **kwargs):
            if closure is not None:
                with torch.enable_grad():
                    closure()
            return original_step(*args, **kwargs)

        optimizer.step = step_with_closure
        return optimizer
    ...
    def configure_optimizers(self):
        if self.optimizer_name == "adam":
            ...
            try:
                ...
                optimizer = self._make_muon_closure_compatible(optimizer)
            except (ImportError, AttributeError) as exc:
                rank_zero_warn(
                    "Requested optimizer='muon' but MuonWithAuxAdam is unavailable "
                    f"from installed muon package ({exc}). Falling back to AdamW."
                )
                optimizer = torch.optim.AdamW(self.model.parameters(), lr=self.lr, betas=(0.9, 0.95), weight_decay=0.01)
        else:
