...
# helpers
from modules.models.SFNO.layers import MLP, DropPath, RealFFT2, SpectralAttention2d, spectral_norm
from modules.models.SFNO.s2convolutions import SpectralAttentionS2, SpectralConvS2, DynamicSpectralConvS2
...
class FilteredFourierNeuralOperatorBlock(nn.Module):
    """Fourier Neural Operator Block"""
    ...
    def __init__(
        ...
        super(FilteredFourierNeuralOperatorBlock, self).__init__()

        # MLP layer

        self.linear_1 = spectral_norm(nn.Conv2d(embed_dim, int(embed_dim * mlp_ratio), 1, bias=False))
        self.activation = act_layer()
        self.linear_2 = spectral_norm(nn.Conv2d(int(embed_dim * mlp_ratio), embed_dim, 1, bias=False))

        # convolution layer
        self.filter = DynamicSpectralConvS2(forward_transform=forward_transform,
        ...
        # gate: initialized to 0 so sigmoid(0)=0.5, equal blend at start
    ...
    def __init__(
        ...
        for i in range(self.encoder_layers):
            encoder_modules.append(
                spectral_norm(nn.Conv2d(current_dim, encoder_hidden_dim, 1, bias=True))
            )
            encoder_modules.append(self.activation_function())
            current_dim = encoder_hidden_dim

        encoder_modules.append(
            spectral_norm(nn.Conv2d(current_dim, self.embed_dim, 1, bias=False))
            )
        ...
        for i in range(self.num_layers):
            ...
            block = FilteredFourierNeuralOperatorBlock(
                forward_transform,
                inverse_transform,
                self.embed_dim,
                mlp_ratio=mlp_ratio,
        ...
        for i in range(self.encoder_layers):
            decoder_modules.append(
                spectral_norm(nn.Conv2d(current_dim, decoder_hidden_dim, 1, bias=True))
            )
            decoder_modules.append(self.activation_function())
            current_dim = decoder_hidden_dim

        decoder_modules.append(
            spectral_norm(nn.Conv2d(current_dim, self.out_chans, 1, bias=False))
            )
