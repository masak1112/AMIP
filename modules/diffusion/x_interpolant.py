...
class DynamicInterpolant(nn.Module):
    ...
    def _apply_noise_scales(self, noise):
        """Apply per-channel noise scales while tolerating channel-count drift.

        Some checkpoints/configs carry legacy noise-scale tensors with different
        channel counts than the current runtime state. We align by channel index:
        truncate extra scales, and pad missing scales with ones (no scaling).
        """
        if self.noise_scales is None:
            return noise

        scales = self.noise_scales.to(device=noise.device, dtype=noise.dtype)
        if scales.ndim == 1:
            scales = scales.view(1, -1, 1, 1)
        elif scales.ndim == 4:
            pass
        else:
            # Fallback for uncommon stored shapes (e.g., [1, C] or [C, 1, 1]).
            scales = scales.view(1, -1, 1, 1)

        noise_channels = noise.shape[1]
        scale_channels = scales.shape[1]

        if scale_channels != noise_channels:
            aligned = torch.ones(
                (1, noise_channels, 1, 1),
                device=noise.device,
                dtype=noise.dtype,
            )
            n_copy = min(noise_channels, scale_channels)
            aligned[:, :n_copy] = scales[:, :n_copy]
            scales = aligned
    ...
    def sample_ddim(self, model, x, c_grid, num_steps=None, gamma=0.0,
        ...
        for i in range(num_steps):
            ...
            xi = self.get_noise(x)
            xi = self._apply_noise_scales(xi)
    ...
    def sample_uniform(self, model, x, c_grid, num_steps=None, return_model_last=False):
        ...
        t_churn_lo = getattr(self, 't_churn_min', 0.2)
        t_churn_hi = getattr(self, 't_churn_max', 0.8)
        ...
        for i in range(num_steps_drift):
            ...
            if a_step > 0 and i > 0:
                ...
                if t_churn_lo <= t_val <= t_churn_hi:
                    ...
                    eps = self._apply_noise_scales(eps)
