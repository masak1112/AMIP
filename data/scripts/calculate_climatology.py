...
def update(existingAggregate, newValues):
    ...
    # count is int
    # mean in shape (c) or (nlevel, c)
    # M2 in shape (c) or (nlevel, c)
    # newValues in shape (reduce_dim, c) or (reduce_dim, nlevel, c)
    
    count += np.sum(~np.isnan(newValues), axis=0) # c
    # newvalues - oldMean
    delta = newValues - np.expand_dims(mean, axis=0) # reduce_dim, c - 1, c
    mean += np.nansum(delta / np.expand_dims(count, axis=0), axis=0)
    # newvalues - newMeant
    delta2 = newValues - np.expand_dims(mean, axis=0) 
...
def calculate_mean_std(f):
    ...
    forcing_mean = reduce_list(surface_mean_list)
    forcing_std = reduce_list(surface_std_list)
