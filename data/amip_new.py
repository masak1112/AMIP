...
H5_VARIABLE_ALIASES = {
    'DSWRFtoa_24h': 'DSWRFtoa_24h_lead',
}

STATS_VARIABLE_ALIASES = {
    'DSWRFtoa_24h_lead': 'DSWRFtoa_24h',
}


def _resolve_stats_variable_name(dataset, variable):
    if variable in dataset:
        return variable

    alias = STATS_VARIABLE_ALIASES.get(variable)
    if alias is not None and alias in dataset:
        return alias

    reverse_alias = next(
        (name for name, mapped_name in STATS_VARIABLE_ALIASES.items()
         if mapped_name == variable and name in dataset),
        None,
    )
...
def get_data_given_path(path, variables):
    ...
    with h5py.File(path, 'r', locking=False) as f:
        ...
        for variable in variables:
            if variable in data:
                stacked.append(data[variable])
                continue

            alias = H5_VARIABLE_ALIASES.get(variable)
            if alias is not None and alias in f['input']:
                stacked.append(np.array(f['input'][alias]))
                continue
...
class GetDataset(Dataset):
    ...
    def __init__(self, params: dict, 
        ...
        print("Dataset initialization: train =", self.train, ", validate =", self.validate)
        ...
        # CO2 is only routed through the calendar tensor when it appears as the
        # first varying boundary channel; otherwise the calendar carries only
        # (sod, doy) and the boundary tensor is left untouched.
        self.has_co2 = (
            len(self.varying_boundary_variables) > 0
            and self.varying_boundary_variables[0] == 'global_mean_co2'
        )
    ...
    def _load_mean_std(self, mean_file, std_file, datavars, upper_air=True):
        ...
        engine = None
        if 'h5netcdf' in available_engines:
            engine = 'h5netcdf'
        elif 'netcdf4' in available_engines:
            engine = 'netcdf4'

        open_kwargs = {'engine': engine} if engine is not None else {}

        if upper_air:
            with xr.open_dataset(mean_file, **open_kwargs) as ds:
                level_mask = xr.DataArray(
                    data=[lev in self.levels for lev in ds['level'].values], dims=['level']
                )
                mean = torch.stack([
                    torch.tensor(
                        ds[_resolve_stats_variable_name(ds, var)].where(level_mask, drop=True).values
                    ).to(torch.float32)
                    for var in datavars
                ], dim=0)
            with xr.open_dataset(std_file, **open_kwargs) as ds:
                level_mask = xr.DataArray(
                    data=[lev in self.levels for lev in ds['level'].values], dims=['level']
                )
                std = torch.stack([
            ...
            with xr.open_dataset(mean_file, **open_kwargs) as ds:
                mean = torch.stack([
                    torch.tensor(ds[_resolve_stats_variable_name(ds, var)].values).to(torch.float32)
                    for var in datavars
                ], dim=0)
            with xr.open_dataset(std_file, **open_kwargs) as ds:
                std = torch.stack([
    ...
    def _getitem_train(self, index, has_boundary, has_diagnostic):
        ...
        if self.return_calendar:
            ...
            if self.has_co2:
                ...
                calendar = torch.tensor([sod, doy], dtype=torch.float32)
    ...
    def _getitem_train_multistep(self, index, has_boundary, has_diagnostic):
        ...
        if self.return_calendar:
            # Per-step calendar (T, 3 or T, 2): [sod, doy, (co2)]. When
            # global_mean_co2 leads varying_boundary_variables, channel 0 is
            # split off into the calendar.
            n_cal = 3 if self.has_co2 else 2
            if self.has_co2:
                co2_per_step = varying_boundary_data[:, 0, 0, 0].clone()
                varying_boundary_data = varying_boundary_data[:, 1:, :, :]
            cal = torch.empty(len(step_times), n_cal, dtype=torch.float32)
    ...
    def _getitem_autoregressive(self, index, lead_times, has_boundary, has_diagnostic):
        ...
        print("start time in inference", start_time)
        data_in = self._get_data(start_time, out=False)
        ...
        if self.return_calendar:
            ...
            if self.has_co2:
                co2_per_step = varying_boundary_data[:, 0, 0, 0].clone()
                varying_boundary_data = varying_boundary_data[:, 1:, :, :]
            calendar = torch.empty(len(step_times), n_cal, dtype=torch.float32)
            for i, t in enumerate(step_times):
                sod, doy = self._compute_calendar(t)
                calendar[i, 0] = sod
                calendar[i, 1] = doy
